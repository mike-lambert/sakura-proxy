...
public class TorClient {
    ...
    public List<Integer> getSocksPorts() {
        return socksListener.listeningPorts();
    }

    public int getPrimarySocksPort() {
        if (socksListener.listeningPorts().isEmpty()) {
            throw new TorException("No active SOCKS5 listeners running");
        }
        return socksListener.listeningPorts().get(0);
    }
}
