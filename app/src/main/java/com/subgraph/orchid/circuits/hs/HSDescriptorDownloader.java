...
public class HSDescriptorDownloader {
    private static final Logger logger = Logger.getInstance(HSDescriptorDownloader.class);
    ...
    private HSDescriptor downloadDescriptorFrom(HSDescriptorDirectory dd) {
        ...
        try {
            ...
            if (http.getStatusCode() == 200) {
                ...
                logger.debug("HS descriptor download for " + hiddenService.getOnionAddressForLogging() + " failed with status " + http.getStatusCode());
            }
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            return null;
        } catch (TimeoutException e) {
            logger.debug("Timeout downloading HS descriptor from " + dd.getDirectory(), e);
            return null;
        } catch (IOException e) {
