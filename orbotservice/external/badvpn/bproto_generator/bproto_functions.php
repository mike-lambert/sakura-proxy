...
#include "{$directive["file"]}"
    ...
    $msg) {
        ...
        static void
        { $msg["name"] }
        Writer_Init({$msg["name"]}
        Writer * o, uint8_t * out);
        static int
        { $msg["name"] }
        Writer_Finish({$msg["name"]}
        Writer * o);
        ...
        static int
        { $msg["name"] }
        Parser_Init({$msg["name"]}
        Parser * o, uint8_t * buf, int
        buf_len);
        static int
        { $msg["name"] }
        Parser_GotEverything({$msg["name"]}
        Parser * o);
        ...
        $entry) {
                    ...
                    {$decl}
            {
                ...
                while (left > 0) {
                    ...
                    o->
                    { $entry["name"] }
                    _pos += sizeof(struct BProto_header_s);
                    left -= sizeof(struct BProto_header_s);
                    uint16_t type = ltoh16(header.type);
                    uint16_t id = ltoh16(header.id);
                    ...
                    switch (type) {
                        ...
                        case BPROTO_TYPE_CONSTDATA: {
                            ...
                            o->
                            { $entry["name"] }
                            _pos += sizeof(struct BProto_data_header_s);
                            left -= sizeof(struct BProto_data_header_s);
                            ...
                            EOD;
                            if ($entry["type"]["type"] == "data" ||
                                $entry["type"]["type"] == "constdata") {
                                echo << < EOD
                                uint8_t *payload = o->buf + o->{ $entry["name"] }
                                _start + o->
                                { $entry["name"] }
                                _pos;
